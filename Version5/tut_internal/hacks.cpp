...
void* NoClipAddrTarget = (void*)(exeBaseAddress + 0xC0AA3);

byte originalMapBytes[6] = { 0 };
byte originalRadarBytes[6] = { 0 };
byte originalNoClipBytes[2] = { 0 };
...
{
	DWORD oldProtect;
	VirtualProtect(NoClipAddrTarget, 2, PAGE_EXECUTE_READWRITE, &oldProtect);
	memcpy(originalNoClipBytes, NoClipAddrTarget, 2);
	*((BYTE*)NoClipAddrTarget) = 0x90;
	*((BYTE*)NoClipAddrTarget + 1) = 0x90;
	VirtualProtect(NoClipAddrTarget, 2, oldProtect, &oldProtect);
}
...
void hacks::unnoClip()
{
	DWORD oldProtect;
	VirtualProtect(NoClipAddrTarget, 2, PAGE_EXECUTE_READWRITE, &oldProtect);
	memcpy(NoClipAddrTarget, originalNoClipBytes, 2);
	VirtualProtect(NoClipAddrTarget, 2, oldProtect, &oldProtect);
}