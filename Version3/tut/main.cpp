...
{
	RECT rect;
	HWND game = FindWindowA(NULL, "AssaultCube");
	...
	GLFWwindow* window = glfwCreateWindow(width, height, "AssaultCube ESP", NULL, NULL);
	...
	while (true)
	{
		...
		for (Entity e : entities)
		{
			...
			ImVec4 rect = m.CalcRect(wtsFeet, wtsHead);

			if (wtsFeet.x > 0)
				if (localPlayer.team != e.team)
				{
					ImGui::GetForegroundDrawList()->AddLine(ImVec2(width / 2, height), wtsFeet, ImColor{255,255,255}, 1.0f);
					ImGui::GetForegroundDrawList()->AddRect({ rect.x,rect.y}, {rect.w,rect.z}, ImColor{ 255,255,255 });
					ImVec2 start{ rect.x + 5, wtsFeet.y };
					ImVec2 end{ rect.x + 5, wtsFeet.y + (wtsHead.y - wtsFeet.y) * (e.health / 100.f) };
